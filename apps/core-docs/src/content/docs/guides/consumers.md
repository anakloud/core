...
| PedConnect server | `pedconnect/apps/pedconnect-server/src/programs/program.service.ts` | Active-service and individual-service reads | Uses `GET /services?active=true` and `GET /services/:id`. |
